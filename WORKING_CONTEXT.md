...
- 기록 시각: 2026-08-01 22:37:08
- 작업 요청 요약: 가스샷 UI 적용 뒤 촬영한 메인·장비·박물관·업적·연구 화면 5장의 잔여 겹침과 잘못된 배치를 진단하고, `Image.Type.Sliced`만 지정해 표면이 압축되는 문제를 막는 상세 교정 프롬프트 TXT를 작성한다.
- 수정 전 상태: 사용자 캡처에서 메인 하단 내비게이션과 강화 카드 부속이 가장자리에 붙고, 장비·수집 모달 타이틀과 닫기 버튼이 겹치며, 장비 비교 트레이의 내부 칸이 압축됐다. 업적은 아이콘이 제목을 가리고 보상 수치가 수령 버튼 뒤로 들어갔으며, 박물관 잠금 오버레이와 연구 행 정보도 과밀했다. 코드 감사 결과 Task13 자산은 `MineUiSkin.Task13Simple()`이 Border 0 Sprite를 만들고 `ApplyStretchedSimpleSprite()`가 `preserveAspect=false`로 확대하며, Task13 전용 Sliced 생성 경로가 없었다.
- 수정 내용: 루트에 `POCKET_FORGE_UI_OVERLAP_AND_9SLICE_CORRECTION_PROMPT.txt`를 생성했다. 5개 화면별 실패 지점과 교정 계약, 1080×1920 너비 기준/1440×3088 검수 좌표, Safe Area·최소 간격·터치 크기, 장비 모달 영역, 업적 5열 비율, 박물관 잠금 카드, 연구 행, 메인 하단 내비게이션 규칙을 기록했다.
- 9-slice 계약: `Image.Type.Sliced`만 바꾸는 작업을 금지하고 실제 pixel Border를 받는 Task13 전용 Sliced 캐시 경로, 알파 트림 후 Border 측정, `target size >= border sum + 8~16px center` 최소 크기 공식, Simple/Sliced/Filled/분리 조립 분류를 요구했다. 내부 구분선이 포함된 장비 비교 트레이는 빈 Sliced 배경과 고정 Divider·런타임 콘텐츠로 분리하도록 했다.
- 검증 계약: Sprite Border·최소 Rect·렌더 타입, 반복 카드 동일 크기, 타이틀/닫기·아이콘/제목·보상/수령 버튼 등 금지 교차, 16:9·19.5:9·20:9·1440×3088 Safe Area, 한·영·일·중 최장 문자열, 동일 해상도 오버레이와 최소/기준/최대 Sliced 렌더를 검사하도록 명시했다.
- 수정 후 상태: 다른 작업에서 바로 사용할 수 있는 736줄 교정 프롬프트가 준비됐다. 현재 Unity 코드·씬·RectTransform·Sprite Border·런타임 자산과 Task 13-5B 상태는 변경하지 않았다.
- 검증 결과: 새 TXT가 UTF-8로 정상 표시되고 32,735바이트이며 메인·장비·박물관·업적·연구 5개 교정 섹션, 9-slice 원인·구현·측정·최소 크기·분류, 자동 테스트·시각 검수·금지·완료 보고 섹션을 포함하는 것을 확인했다. 문서 작성 작업이므로 Unity 컴파일·EditMode·Play Mode·Device Simulator·Android 검증은 수행하지 않았다.
- 남은 작업: 실제 교정은 이 TXT를 입력으로 Task13Sliced 경로와 자산별 Border를 측정·적용하고 화면별 RectTransform을 수정한 뒤, 5개 화면과 다중 화면비·다국어 캡처를 다시 검증하는 별도 승인 작업으로 수행한다.

## 수정사항 60

- 기록 시각: 2026-08-02
- 작업 요청 요약: `POCKET_FORGE_UI_OVERLAP_AND_9SLICE_CORRECTION_PROMPT.txt`를 실행 기준으로 삼아 메인·장비·박물관·업적·연구 화면의 겹침과 잘못 늘어난 표면을 실제 코드에서 교정한다. 필요하면 외부 패키지·스킬을 조사하되 배치·비율 문제에 실익이 있을 때만 추가한다.
- 조사 결과: 공식 Codex 스킬 목록과 Unity UGUI 공식 자료를 확인했으나 현재 문제를 대신 해결할 전용 패키지·추가 스킬은 없었다. 기존 `pocket-forge-ui-layout-guard`, Unity MCP, UGUI의 RectTransform·Sprite Border로 범위를 유지했으며 외부 패키지는 설치하지 않았다.
- 9-slice 구현: `MineUiSkin.Task13Sliced()`가 실제 픽셀 Border를 가진 Sprite를 캐시하고 잘못된 값은 경고 후 Simple로 되돌리도록 했다. `ApplyBorderedSprite()`는 Border 축과 `border 합 + center 최소 크기`를 Assert하고 Sliced 설정을 한 경로에서 적용한다. 장비 비교 트레이는 구분선이 구워진 원본을 늘리지 않고 균일 배경, 두 Divider, 선택/차이/현재 3열로 분리했다.
- 레이아웃 교정: 장비·수집 모달은 920 폭 공통 헤더와 충분한 세로 공간을 사용하고, 장비 4슬롯·2×3 인벤토리·비교·행동을 분리했다. 박물관 잠금 오버레이는 광석 표시의 45% 이하로 줄였고 업적은 아이콘/목표/보상 아이콘/보상 값/상태의 고정 열을 사용한다. 연구는 920 폭으로 넓히고 목록 루트 피벗을 수정해 제목·요약 아래에서 시작하며 텍스트 62%와 버튼 30%를 분리했다. 메인은 보스 버튼을 전용 중앙 행으로 이동하고 하단 내비게이션을 Safe Area 안의 6개 동일 간격 셀로 축소했다.
- 렌더 타입: 모달·장착 슬롯·박물관 카드·탭·행동 버튼은 측정 Border Sliced, 내부 선이 있는 인벤토리 카드·업적 행·요약·제목·아이콘은 고정 비율 Simple, 진행 채움은 Filled다. 비교 트레이는 분리 조립이다. 원본 크기·Border·분류 근거는 `Review/Task13OverlapFix/UI_OVERLAP_AND_9SLICE_REVIEW.md`에 기록했다.
- 검증 결과: Unity 컴파일 오류 0건, UI·화면비·한영일중 대상 EditMode 30/30, 전체 EditMode 154/154를 통과했다. 401×726 Game View에서 메인·장비·연구·박물관·업적 5화면을 실제 렌더해 제목/닫기, 잠금/텍스트, 보상/수령, 연구/버튼, 하단 가장자리를 육안 검수했다. 첫 연구 캡처에서 목록 루트가 제목을 덮는 문제를 발견해 피벗 좌표를 고친 뒤 재캡처했다.
- 검증하지 않은 항목: Android APK/AAB 빌드와 실기기 테스트는 수행하지 않았다. 캡처를 위해 Play Mode Canvas를 일시적으로 Screen Space Camera로 바꿨지만 런타임 저장 자산에는 반영하지 않았다.
- 다음 작업: `TASKS.md`의 Task 13-5B 일일·주간 미션 기획 조율과 구현으로 돌아간다.

## 수정사항 61

- 기록 시각: 2026-08-02
- 작업 요청 요약: 사용자가 Play Mode에서 직접 수정한 수십 개 HUD 배치·크기·삭제 상태를 모두 기록한 뒤 Play Mode를 종료하고, 같은 화면이 다음 실행에도 재현되도록 생성 코드에 영구 반영한다. 설정 모달과 게임 상태에 따라 변하는 값은 변경하지 않는다.
- 기준 캡처: 1440×3088 Play Mode HUD 스크린샷과 SafeArea 하위 RectTransform 125개의 경로·형제 순서·활성 상태·앵커·피벗·좌표·크기·스케일·컴포넌트를 `Review/CurrentPlayModeSnapshot_2026-08-02/`에 저장했다. 새 `MineHudView.Create()` 결과와 비교해 89개 경로의 156개 속성 차이, 삭제 11개를 확인했으며, 그중 구조상 영향 범위는 이동·크기·순서 55개 경로와 삭제 11개를 합친 66개 경로다.
- 영구 반영: 상단 초상화·광부 레벨·경험치·3종 재화·광고·설정, 광산명, 체력 Fill, 채굴·보스 버튼, 업그레이드 카드 3종의 아이콘·레벨·가격 요소, 하단 내비게이션 6개 슬롯을 기록 좌표로 변경했다. 미션은 `BottomNavigationBar` 자식 관계를 유지한 채 전투력 아래 `455,1529`에 배치하고, 나머지 5개 메뉴만 하단 행에 남겼다.
- 삭제 유지와 오류 수정: `MineActionText`, `SelectedNavigationTab`, 내비게이션 동적 문구 6개, 업그레이드 이름 3개의 생성·필드·갱신 경로를 제거했다. 삭제된 Text를 `navigationLabels`에서 다시 접근하던 `UpdateNavigationLabels()`도 제거해 `MineHudViewV5.cs:717`의 반복 `MissingReferenceException` 원인을 해소했다.
- 진행바: `ProgressFill`의 왼쪽 앵커·피벗과 x=16을 유지하고 높이 53을 반영했으며, 사용자 요청대로 기준 폭을 488에서 468로 20 줄였다. 경험치 진행 폭, 잠금·알림 활성, 오프라인 보상 활성, 성공 파티클 좌표는 게임 상태 또는 애니메이션 값이므로 고정하지 않았다.
- 회귀 방어: `MineHudResponsiveLayoutTests`에 새 상단·내비게이션·진행바·채굴 아이콘 좌표와 의도적 삭제 객체 부재 검사를 추가하고, 이전 좌표를 전제로 한 두 테스트를 새 승인 기준으로 갱신했다.
- 검증 결과: Unity 스크립트 강제 재컴파일 후 Console 오류 0건, `MineHudResponsiveLayoutTests` EditMode 30/30과 전체 EditMode 154/154 통과를 확인했다. Play Mode는 사용자 요청대로 종료 상태를 유지했으며 재실행 캡처, Android APK/AAB, 실기기 검증은 수행하지 않았다.
- 다음 작업: 현재 UI 기준을 유지한 채 `TASKS.md`의 Task 13-5B 일일·주간 미션 기획 조율과 구현으로 돌아간다.

## 수정사항 62

- 기록 시각: 2026-08-02
- 작업 요청 요약: 장비·박물관을 포함한 Task 13 UI 전반에서 둥근 모서리보다 안쪽을 자른 잘못된 9-slice, 서로 맞지 않는 카드 베이스·오버레이, 카드 이미지에 구워진 게이지와 런타임 게이지의 중복을 전면 교정한다.
- 원인 감사: Task13 Sliced 호출 22곳이 실제 곡선과 무관한 24~48px 값을 개별 전달했다. 장비 인벤토리의 290×512 베이스와 268×512 희귀도·선택 오버레이가 같은 190×328 Rect에서 Simple·preserveAspect 설정을 서로 다르게 사용했고, 박물관 288×512 베이스에는 하단 패널과 게이지가 이미 포함돼 있었지만 코드가 트랙·Fill을 다시 생성했다. 장비 비교와 연구 행은 세로 모달 본체를 가로로 눌러 쓰고 있었다.
- 자산 재구축: `imagegen` 제한 편집으로 내부 Divider 없는 장비 카드, 구워진 하단 패널·게이지 없는 박물관 카드, 내부 구획 없는 공용 가로 패널을 생성했다. 생성 결과의 검정 외곽은 테두리에서 연결된 영역만 flood-fill 투명화하고 원본 런타임 크기로 정규화했다. 버튼 5종은 외관을 바꾸지 않고 최소 실제 Rect에 모서리와 16px 중심이 들어오도록 해상도만 축소한 Runtime 사본을 만들었다. 원본 PNG는 유지했다.
- 렌더 정책: `MineUiSkin`에 19종의 측정 Border 카탈로그를 추가하고 `Task13Sliced(assetName)`만 공개해 호출부 임의 값을 제거했다. 장비 베이스·희귀도·선택 테두리는 같은 Stretch Rect와 Sliced 규칙을 사용하며, 비교·연구 행은 공용 가로 패널을 사용한다. 박물관은 클린 카드 위에 발견 항목의 런타임 게이지만 하나 표시하고 미발견은 숨긴다. 박물관 요약과 다음 보상도 내용 없는 장식 슬롯 대신 클린 패널·실제 아이콘·텍스트만 표시한다.
- 회귀 방어: 생성 직후 Stretch 자식의 `sizeDelta == 0`을 실제 폭 0으로 오판하지 않도록 부모 Rect와 anchor span으로 렌더 크기를 계산한다. `Task13UiSliceCatalogTests`는 19종 Resources 존재, Border 카탈로그, 최소 Rect 적합성과 Sprite Border 일치를 검사하고 기존 HUD 분류 테스트는 인벤토리·요약의 새 Sliced 정책으로 갱신했다.
- 시각 검증: 1080×1920과 연결된 세로 기기 비율에서 장비·연구·박물관·업적을 실제 Play Mode로 열어 렌더했다. 장비 카드 세 레이어의 외곽 일치, 연구 행 모서리, 박물관 단일 게이지·클린 요약, 업적 고정 열을 확인했으며 캡처용 Screen Space Camera 변경은 Play Mode 종료로 폐기했다. 결과는 `Review/Task13OverlapFix/`에 보관한다.
- 검증 결과: Unity 강제 컴파일과 최종 Console 오류 0건, UI·화면비·4개 언어·Slice 카탈로그 EditMode 31/31, 전체 EditMode 155/155를 통과했다. Android APK/AAB와 실기기 입력 검증은 이번 범위에서 수행하지 않았다.
- 다음 작업: UI 재구축은 완료했다. `TASKS.md`의 Task 13-5B 일일·주간 미션 설계 조율과 구현으로 돌아간다.

## 수정사항 63

- 기록 시각: 2026-08-02
- 작업 요청 요약: 기존 UI 자산을 우선 사용해 Task 13-5B 일일·주간 미션을 구현하고, 전용 그래픽이 필요한 부분은 임시 마감 후 교체 대상으로 기록했다. 기능 완성도에 도움이 되는 범위에서는 소규모 저장·테스트 리소스를 추가했다.
- 콘텐츠 구조: `MissionDefinition`에 기간·지표·목표·보상 타입을 분리하고 `MiningContentCatalog`의 직렬화 배열과 런타임 기본값으로 일일 4개·주간 4개를 구성했다. 채굴, 시설, 연구, 장비는 기간 시작 스냅샷과 기존 누적 상태의 차이로 진행도를 계산하며 반복 보스 처치만 `bossesDefeated` 누적값을 추가했다.
- 시간 정책: 일일은 UTC 00:00, 주간은 월요일 UTC 00:00에 갱신한다. `lastObservedMissionUnixSeconds`보다 과거인 기기 시각은 마지막 관찰 시각으로 고정해 이전 기간을 다시 열지 않는다. 서버 시간이 없는 로컬 방어이므로 계정 단위 보안 검증은 제공하지 않는다.
- 저장·보상: 저장 버전을 11로 올리고 일일·주간 `MissionPeriodData`, 기간 기준선, 개별 수령 ID, 완료 보상 수령 상태, 마지막 관찰 UTC와 보스 누적을 정규화했다. 개별·완료 수령은 각각 중복을 차단하며 Credits·Gem·설계도 코어·기존 `EquipmentService.GrantBossReward`만 사용한다.
- UI·다국어: 광부 Lv.5 미션 진입점에 일일·주간 탭, 갱신 남은 시간, 4개 공용 행, 기간 완료 보상과 알림 배지를 연결했다. 행은 아이콘·유동 텍스트/진행·보상·고정 수령 버튼 열로 구성하고 한영일중 문구를 추가했다. 기존 Task13 모달·업적 행·버튼·재화 아이콘을 재사용했다.
- 후속 그래픽: 일일·주간 전용 배너, 미션별 전용 아이콘, 설계도 코어 보상 아이콘, 기간 완료 상자·수령 연출은 현재 기능에 필수적이지 않아 기존 자산으로 임시 마감했다. Task 13-6 또는 후속 UI 폴리싱에서 교체한다.
...
- 기록 시각: 2026-08-03
- 작업 요청 요약: Android 빌드 검증을 이후 기본 범위에서 제외하고 Task 14 밸런싱으로 이동한다. 첫 세부 단계에서는 실제 성장 공식으로 챕터 1의 첫 강화·보스 도달·완료 시간을 측정하고 초반 수치를 조정한다.
- 수정 전 상태: 신규 플레이어 자동 채굴력은 0.5/s, 최대 기준 탭 포함 채굴력은 5.5/s였고 챕터 1 보스 권장치도 5.5/s여서 시설·영구 성장 없이 첫 보스를 돌파할 수 있었다. 구리 1스테이지 내구도는 10이고 초반 일반 스테이지 9개가 빠르게 끝나 능동 플레이의 챕터 길이가 방치형 목표보다 짧았다.
- 시뮬레이션 구조: `ProgressionBalanceSimulator`를 추가해 실제 `MiningGameService`의 채굴, 자동 피해, 탭 쿨다운, 강화 구매, 광부 경험치, 도감 배율, 보스 실패와 9스테이지 파밍, 명시적 재도전을 0.2초 단위로 재생한다. 희귀 판정은 발생하지 않도록 고정하고 현재 채굴력 증가량/비용이 가장 높은 시설을 구매하는 동일 전략으로 능동·자동 모드를 비교한다.
- 수치 조정: 카탈로그의 기본 자동 채굴력을 1.5/s, 기본 탭 피해를 0.08로 조정했다. 곡괭이는 비용 6·성장 1.55·효과 0.01, 드릴은 18·1.55·0.35, 로봇은 35·1.55·0.08로 조정했다. 구리 내구도는 25+12/stage, 철 60+15/stage, 금 110+25/stage, 크리스털 기본 60으로 변경하고 챕터 1 보스 배율을 2로 조정해 권장 채굴력을 4.0/s로 만들었다.
- 측정 결과: 능동 모드는 첫 강화 32.6초, 첫 보스 261.0초, 완료 482.8초, 보스 실패 1회, 최종 곡괭이 5/드릴 3/로봇 1과 4.532/s였다. 자동 모드는 첫 강화 113.6초, 첫 보스 384.6초, 완료 983.6초, 보스 실패 1회, 최종 곡괭이 0/드릴 5/로봇 3과 4.275/s였다. 두 모드 모두 기본 상태로는 4.0/s 보스를 잡지 못하고 파밍·투자를 거쳐 재도전한다.
- 회귀 수정: 실제 카탈로그 수치에 의존하던 보스 권장 채굴력, 오프라인 4시간 처리량·XP·레벨·Credits, HUD 채굴력 예상값 테스트를 새 기준으로 갱신했다. 런타임 기본 카탈로그를 사용하는 독립 규칙 테스트 값은 변경하지 않았다.
- 검증 결과: 신규 밸런스 테스트 3/3, 채굴·보스 영향 범위 36/36, 전체 EditMode 179/179를 통과했다. Unity 강제 컴파일 후 C# 컴파일 오류는 없었다. Test Runner가 결과 XML 저장 경로를 Console의 Exception 유형으로 기록했지만 실패나 런타임 예외는 아니며 테스트 요약은 Passed다.
- 미검증: 사용자 요청에 따라 Android APK/AAB 빌드, 설치, 실기기 입력은 수행하지 않았다. 실제 사람의 탭 피로도, 희귀 광석 운, 첫 세션 이탈률은 사용자 플레이와 Play 비공개 테스트에서 별도 측정해야 한다.
- 다음 작업: Task 14-2에서 일일 무료·광고·Gem 교환·스타터 패키지·미션·이벤트 보상을 분당 생산량과 비교해 재화 공급·소비를 조정한다. 이후 Task 14-3에서 챕터 2~3과 오프라인·광고 빈도·장기 세션을 조정한다.

## 수정사항 66

- 기록 시각: 2026-08-06 14:59:07
- 작업 요청 요약: Task 14-2를 바로 구현하지 않고, 현재 경제 감사 결과와 추후 적용할 조정 후보·검증 범위만 문서에 기록한다.
- 수정 전 상태: `TASKS.md`와 `PROJECT_PLAN.md`에는 메타 경제를 초반 생산량과 비교한다는 방향만 있었고, 현재 임시 보상의 생산량 대비 비율이나 구체적인 조정 후보는 기록돼 있지 않았다.
- 기록한 내용: Task 14-1 시뮬레이터를 읽기 전용으로 실행해 능동 챕터 1 총생산 546 Credits, 자동 총생산 799 Credits, 전체 연구 비용 59 코어를 확인했다. 이를 기준으로 현행 무료·광고·Gem·스타터 보상과 주간 코어 공급의 과잉 비율을 계산하고, 상점·미션·업적·이벤트별 조정 후보 및 회귀 테스트 계획을 `TASKS.md`와 `PROJECT_PLAN.md`에 기록했다.
- 수정 후 상태: Task 14-2는 `계획 기록 완료 · 구현 보류` 상태다. 모든 후보 수치는 미적용임을 명시했으며 게임 코드, Unity 데이터, UI, 저장 스키마, 광고·결제 처리는 변경하지 않았다.
- 테스트 결과: 문서 내용과 기존 정의값을 대조했다. Unity 컴파일, EditMode, Play Mode, Android APK/AAB, 실기기 테스트는 문서 기록 작업이므로 실행하지 않았다.
- 남은 작업: 사용자가 실제 구현을 다시 승인하면 후보 수치를 적용하고 총생산·총소비 Credits와 보상 비율, 주간 코어 공급 상한 회귀 테스트를 추가한다. 이후 Task 14-3에서 챕터 2~3과 오프라인·광고 빈도·세션 길이를 조정한다.

## 수정사항 67

- 기록 시각: 2026-08-06 15:25:00
- 작업 요청 요약: 수정사항 66에서 기록만 해 둔 Task 14-2 메타 경제 조정 후보를 사용자 승인에 따라 실제로 적용하고, 총생산·총소비 측정과 메타 경제 회귀 테스트를 추가한다.
- 수정 전 상태: 상점·미션·업적·이벤트 실행값은 Task 14 전 기능 검증용 임시 수치였다. 일일 무료 상점과 광고만으로 능동 챕터 1 생산의 약 6.4배 Credits가 공급되고, 보스 제외 주간 반복 코어 공급이 최대 35개로 연구 총비용 59코어 대비 연구 수명이 2주 미만이었다. `ProgressionBalanceSimulator`는 시간·구매 횟수만 측정하고 Credits 총생산·총소비는 측정하지 않았다.
- 수정한 내용: `ShopProductDefinition`·`MissionDefinition`·`AchievementDefinition`·`MiningEventDefinition`의 `CreateRuntimeDefaults()`와 `MissionService`의 일일 완료 보상을 TASKS.md 조정 후보 그대로 적용했다(일일 무료 125 Credits·코어 제거, 광고 200×3, 5 Gem→600 Credits, 10 Gem→코어 3, 스타터 2,000+20+6, 일일 미션 Credits 100/75/125·완료 코어 1, 주간 600/800·코어 2·Gem 3, 이벤트 누적 250/코어 2/Gem 3·교환 20토큰→코어 1, 업적 채굴 75/300/1,500·시설 100/500/2,000·장비 150/600/2,500). `ProgressionBalanceSimulator`에 `TotalCreditsEarned`·`TotalCreditsSpent`를 추가하고, 신규 `MetaEconomyBalanceTests` 5종으로 생산·소비 보존, 무료+광고 0.5~1.5배, Gem 1.25배·스타터 4배 상한, 주간 반복 코어 13개 상한, 연구 수명 4~5.5주를 회귀로 고정했다. 구 수치를 하드코딩한 기존 테스트 7개 파일(Shop·Mission·Event·Achievement·IapCoordinator·HUD 레이아웃)을 새 값으로 갱신했다.
- 수정 후 상태: 시뮬레이터 측정 능동 546 생산/214 소비, 자동 799 생산/437 소비로 사전 감사와 일치하며 챕터 1 완료 시간(482.8초/983.6초)은 변하지 않았다. 무료 상점+광고 일일 합계는 능동 챕터 생산의 약 1.33배, 보스 제외 주간 반복 코어 공급은 일일 완료 7+주간 미션 2+이벤트 누적 2+교환 2 = 13개다. 저장 스키마·UI 구조·광고·IAP 처리 로직·Unity 자산은 변경하지 않았다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 184/184 통과(신규 5종 포함), 최종 Console 오류는 TestResults.xml 경로 안내 로그 1건 외 0건. 기본 검증 정책에 따라 Android APK/AAB·실기기 검증은 실행하지 않았다.
- 남은 작업: Task 14-3에서 챕터 2~3 성장 곡선, 오프라인 생산 상한, 광고 빈도, 세션 길이를 조정한다. 조정된 상점·미션·이벤트 수치의 실제 체감은 Play 비공개 테스트에서 별도 확인한다.

## 수정사항 68

- 기록 시각: 2026-08-06 15:55:00
- 작업 요청 요약: 전체 화면 시각 검수에서 확인한 메인 HUD P1 결함(겹침·이탈·비율)과 사용자가 지목한 광석 내구도 바 비율을 수정한다.
- 수정 전 상태: 내구도 바 채움(468×53, 트랙 왼쪽 +16)이 `12_OreHealthFrame` 아트가 실제로 그려지는 481 캔버스 폭보다 왼쪽으로 튀어나가 보라 테두리를 덮었고 수치는 소수점(190.62)으로 표시됐다. XP 수치는 방패 아이콘에 첫 글자가 가려졌고 진행 바는 패널 상단 밖에 떠 있었으며 최대 폭 160이 트랙 140을 초과했다. `보스 도전` 문구는 코드에서 `\n`으로 2줄 강제였고, 상단 바의 광고 `+` 버튼(48×48)은 아트에 구워진 빈 알약의 오른쪽 끝에 걸쳐 있었다. 연구 완료 배지는 카드 아트(캔버스 111폭)가 아닌 246폭 렉트 코너에 붙어 허공에 떴고, 내비 알림 배지도 아이콘에서 분리돼 있었다.
- 수정한 내용: 프레임 아트의 오목 영역(908×110 아트 px, 86/174 스케일)을 실측해 트랙·채움을 449×54(-1,0.5)로 정합하고 `OreProgressWidth`를 449로 변경했다. 내구도 수치는 올림 정수로 표시한다. XP는 아이콘(-50,-1)·수치(35,-1, 110×30, BestFit 12~18, `XP` 접미사 제거)·언더라인 트랙(28,-20, 96×8, 채움 최대 88)으로 재배치했다. 보스 도전 문구를 1줄로 바꾸고 BestFit(16~25)을 적용했다. `+` 버튼을 알약 중앙(308,-190)으로 옮기고 터치 영역을 100×62로 확장했다. 연구 완료 배지를 (-76,-10), 연구 내비 배지를 (-38,-20), 미션 내비 배지를 (-28,-12)로 아이콘·카드 코너에 정렬했다. 레이아웃 핀 테스트의 해당 좌표를 갱신했다.
- 수정 후 상태: 1440×3088 Play Mode 캡처(`Review/UiPass_Before_Full.png` → `Review/UiPass_Final_Full.png`)에서 채움이 프레임 안쪽에 균등 여백으로 들어가고, XP 수치·언더라인이 알약 안에 정렬되며, 보스 도전 1줄·`+` 중앙 배치·배지 밀착을 육안 확인했다. TASKS.md에 Task 15(UI 정합성 패스) 섹션과 P2/P3 잔여 목록을 기록했다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 184/184 통과. Android APK/AAB·실기기 검증은 기본 검증 정책에 따라 실행하지 않았다.
- 남은 작업: Task 15 P2/P3(버튼 스타일 통일, 재화 아이콘 규격화, 미션 행 간격, 카드 아트 빈 게이지 제거 등)는 승인 후 진행한다.

## 수정사항 69

- 기록 시각: 2026-08-06 16:40:00
- 작업 요청 요약: 메인 HUD에 이어 장비·연구·상점·이벤트·박물관·업적·일일·주간 미션 화면을 전수 검수하고 발견된 겹침·간격 문제를 수정한다.
- 수정 전 상태: 장비 인벤토리 카드의 합성 가능 마름모 배지(-30,-74)가 카드의 그려지는 테두리 안쪽 경계를 넘어 튀어나와 보였다. 미션 모달은 행 피치 250 대비 실제 그려지는 행 높이가 약 174라 행 사이가 약 76씩 벌어졌고, 기간 완료 보상 행은 카드 하단(y 208)에 떨어져 있었으며 일일 완료 보상 텍스트가 "1 설계도 / 코어"로 단어 중간에서 줄바꿈됐다.
- 수정한 내용: Play Mode에서 8개 화면을 1440×3088로 캡처(`Review/Modal_*.png`)해 검수했다. 장비 MergeBadge를 (-40,-76)으로 인셋했다. 미션 행을 y 330/130/-70/-270(피치 200)으로 압축하고 완료 보상 행을 y 387로 올려 목록 바로 아래에 붙였으며, 설계도 코어 보상 문자열의 공백을 비분리 공백으로 바꿔 수량과 이름이 한 단위로 줄바꿈되게 했다. 상점·이벤트·업적·박물관·연구는 구조 양호로 판정하고 Task 14-2 수치(일일 무료 125C, 광고 200C, 미션 100/75/125, 업적 75/100/150, 이벤트 250C/2코어/20토큰→1코어, 완료 코어 1)가 UI에 정상 표시됨을 확인했다.
- 수정 후 상태: 재캡처(`Modal_Equipment_Fixed.png`, `Modal_MissionsDaily_Fixed.png`)에서 마름모 배지가 카드 안에 들어오고 미션 4행+완료 행이 균일 간격으로 붙어 표시되며 완료 보상이 "미션 4개 완료 보상: / 1 설계도 코어"로 깔끔하게 줄바꿈됨을 확인했다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 184/184 통과. Android 검증은 기본 검증 정책에 따라 미실행.
- 남은 작업: 연구 모달 하단 여백, 상점 재화 표기 통일, 버튼 스타일 3계열 통일 등 Task 15 P2/P3 항목은 승인 후 진행한다.

## 수정사항 70

- 기록 시각: 2026-08-06 17:30:00
- 작업 요청 요약: Task 15 P2 UI 정합성 항목(버튼 스타일 통일, 헤더 패널 정렬, 업그레이드 카드 빈 게이지, 연구 모달 여백, 상점 재화 표기, 박물관 닫기 터치 영역)을 적용한다.
- 수정 전 상태: 챕터 패널은 636x188 렉트에서 아트가 636x144.9로, 전투력 패널은 306x188 렉트에서 306x167.6으로 그려져 같은 y에 배치됐는데도 높이가 22.7 달라 위아래로 어긋나 보였다. 주 행동 버튼이 연구 구매·장비 장착에는 주황 육각형, 미션·업적·상점에는 금색 알약으로 갈렸다. 업그레이드 카드 아트에는 레벨과 비용 행 사이에 아무 데이터도 연결되지 않은 오목한 빈 게이지가 구워져 있었다. 연구 카드는 1200 높이에 행 3개만 있어 행과 닫기 버튼 사이에 약 240의 빈 공간이 남았다. 상점·이벤트의 설계도 코어 수량은 하드코딩된 영문 `CORE`로 표시돼 4개 언어 어디에서도 현지화되지 않았다. 박물관 닫기 버튼만 72 높이로 다른 모달(82~88)보다 납작했다.
- 수정한 내용: 전투력 패널을 265x188·x 353으로 조정해 아트가 265x145.1로 그려지게 하고 내부 텍스트 3종의 오프셋·박스·폰트를 같은 265/306 비율로 축소했다. 연구 구매 버튼과 장비 주 행동 버튼의 표면을 `ButtonEquipmentEquipRuntime`(주황 육각형)에서 `ButtonAchievementClaimRuntime`(금색 알약)으로 교체해 주 행동 표면을 하나로 통일했다. 업그레이드 카드의 빈 게이지 위치(로컬 0,-82)에 `UpgradeEffect` 텍스트를 추가하고 기존 `tap`·`auto`·`reward` 키로 각 시설의 현재 기여도(탭 피해, 드릴 자동 채굴력, 로봇 보상 배율)를 표시했다. 연구 카드 높이를 1040으로 줄이고, `blueprint_core_short` 키를 4개 언어에 추가해 상점·이벤트의 `CORE`를 대체했으며, 박물관 닫기 버튼을 360x88·y 62로 키웠다.
- 수정 후 상태: 1440×3088 캡처(`Review/P2_*.png`)에서 두 헤더 패널의 상하단이 일치하고, 업그레이드 카드가 `탭 6.33`·`자동 9.2`·`보상 ×2.04`를 표시하며, 연구 모달의 빈 공간이 사라지고, 이벤트가 `2 코어`, 스타터 패키지가 `2K C + 20 ◆ + 6 코어`로 현지화됨을 확인했다. UI에서 주황 육각형 버튼이 사라졌다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 186/186 통과. 신규 회귀 2종(헤더 패널 그려지는 높이 일치, 업그레이드 기여도 텍스트의 레벨·비용 행 비침범)을 추가했다. 중간에 `AssertNoOverlap`의 message 인자 누락으로 테스트 어셈블리 컴파일이 실패해 이전 어셈블리로 184개만 실행되던 상태를 발견하고 교정했다. Android 검증은 기본 검증 정책에 따라 미실행.
- 남은 작업: Task 15 P3(재화 아이콘 규격화, 방치 보상·연구 바로가기 좌우 규격 통일, 모달 최소 폰트 규칙)은 승인 후 진행한다.

## 수정사항 71

- 기록 시각: 2026-08-06 18:20:00
- 작업 요청 요약: Task 15 P3 UI 정합성 항목(재화 아이콘 규격화, 방치 보상·연구 바로가기 카드 규격 통일, 모달 공통 최소 폰트 규칙)을 적용한다.
- 수정 전 상태: 재화 아이콘 아트의 종횡비가 크게 달라(코인 0.98, 젬 1.11, 설계도 코어 1.77) 같은 정사각 박스에 `preserveAspect`로 그리면 코어가 코인의 약 57% 면적으로만 그려져 유독 작아 보였다. 방치 보상 카드는 제목·아이콘·값 2줄 구조인데 연구 바로가기는 제목과 큰 아이콘만 있어 좌우가 짝이 맞지 않았다. 텍스트 최소 크기 기준이 없어 전투력 권장치 16, 전투력 라벨·업그레이드 기여도·장비 수량 배지가 17로 렌더됐다.
- 측정 근거: 아이콘 4종의 불투명 영역을 픽셀 측정해 종횡비를 확인했고, Play Mode에서 4개 언어 전환하며 모든 활성 Text의 `fontSizeUsedForBestFit`를 읽어 18 미만인 11개를 특정했다. BestFit로 축소된 텍스트는 어떤 언어에서도 없었다.
- 수정한 내용: 그려지는 면적을 같게 맞추는 `SizeCurrencyIcon` 헬퍼를 추가해 미션 행 보상·미션 완료 보상·업적 행 보상 아이콘에 적용했다. 업적 보상 아이콘은 stretch 앵커라 절대 크기가 적용되지 않아 중앙 앵커 50×50으로 변경했다. 상단 바는 아트에 구워진 알약 폭이 이미 꽉 차 있어 확대할 공간이 없으므로 제외했다. 연구 바로가기 아이콘을 방치 카드와 같은 112×92·y 8로 맞추고 하단에 연구 배율(`+0%`)을 표시하는 값 텍스트를 y -56에 추가했다. `MineHudView.MinimumReadableFontSize = 18` 상수를 두고 기준 미달 텍스트 4종을 18로 올렸다.
- 수정 후 상태: 1440×3088 캡처(`Review/P3_*.png`)에서 주간 미션의 코어 아이콘이 코인·젬과 비슷한 무게로 보이고, 업적 보상 열의 코어도 동일하게 교정됐으며, 방치 보상과 연구 카드가 제목·아이콘·값의 같은 3단 구조로 정렬됨을 확인했다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 188/188 통과. 신규 회귀 2종을 추가했다. 하나는 모달 5종을 모두 연 상태로 4개 언어를 순회하며 모든 활성 라벨이 18 이상으로 해석되는지 검사하고, 다른 하나는 주간 미션 보상 아이콘 4개가 종횡비가 서로 다름에도 같은 그려지는 면적을 갖는지 검사한다. 첫 작성 때 모달을 열지 않아 장비 수량 배지 17이 검사에서 누락된 것을 발견하고 테스트 범위와 수치를 함께 교정했다. Android 검증은 기본 검증 정책에 따라 미실행.
- 남은 작업: Task 15는 P1·P2·P3 모두 완료했다. 다음은 Task 14-3 중장기 밸런싱이다.

## 수정사항 72

- 기록 시각: 2026-08-06 19:30:00
- 작업 요청 요약: Task 14-3 중장기 밸런싱 — 챕터 2~3 성장 곡선, 오프라인 생산, 광고 빈도, 세션 길이를 조정한다.
- 측정으로 확인한 문제: 광석 정의가 stage 10(crystal)까지만 존재해 챕터 2~3이 crystal의 `60 + 7/스테이지` 선형 곡선을 그대로 이어 썼다. 그 결과 stage 9(금, 내구도 160)에서 stage 11(67)로 넘어갈 때 일반 광석이 58% 약해지는 역전이 있었다. 또한 보상은 `stage × 광석배율`로 선형인데 강화 비용은 `1.55^레벨` 기하급수라 챕터가 진행될수록 경제가 비용을 따라가지 못했다. 다챕터 시뮬레이션 결과 아이들 경로는 챕터 2에 3138초(52.3분), 챕터 3은 7200초 안에 클리어하지 못했고, 능동은 483/1028/2328초로 챕터마다 2배씩 늘었다. 보스 권장 채굴력도 4.0 → 15.17(3.8배) → 26.67(1.76배)로 챕터 1→2에만 절벽이 있었다.
- 시뮬레이터 확장: `SimulateChapters(mode, throughChapter, max)`와 `ChapterProgressResult`를 추가해 챕터별 소요 시간, 완료 시각, 보스 실패 수, 처리 광석 수, 획득 Credits, 보스 권장치, 클리어 시점 채굴력을 각각 기록한다. 기존 `SimulateFirstChapter`는 `SimulateChapters(mode, 1)` 래퍼로 유지해 Task 14-1 기준값과 테스트를 보존했다. 보스 재도전 로직이 챕터 1로 하드코딩돼 있던 것을 현재 챕터 기준으로 일반화했다.
- 설계 판단: 챕터 2~3 전용 광석 정의를 추가하는 방안은 박물관이 2×2 그리드에 `EnsureMuseumRows`로 행을 쌓는 구조여서 광석이 6종이 되면 3번째 행이 다음 보상 요약 패널과 겹치는 것을 확인하고 배제했다. 대신 `ChapterDefinition.rewardMultiplier`를 신설해 `GetOreReward`에 곱하는 방식으로, 광석 종류를 늘리지 않고 챕터 단위 경제 곡선을 세웠다.
- 수정한 내용: 카탈로그에서 챕터 2를 보상 `×1.5`·보스 내구도 `×3.5 → ×1.65`, 챕터 3을 보상 `×4`·보스 내구도 `×4.0 → ×1.85`로 조정하고 crystal 광석의 스테이지당 내구도를 `7 → 14`로 올렸다. 전면 광고는 `MineGameController`에 하드코딩돼 있던 `new InterstitialAdPolicy(5, 180f)`를 카탈로그 필드 3종으로 분리하고 `InterstitialAdPolicy`에 신규 플레이어 유예 구간을 추가해 간격 5광석 유지, 쿨다운 240초, 유예 600초로 설정했다.
- 수정 후 상태: 아이들은 984/1638/3366초(16.4/27.3/56.1분), 능동은 483/624/750초(8.0/10.4/12.5분)로 두 경로 모두 챕터 길이가 단조 증가한다. 보스 권장 채굴력은 4.0 / 11.0 / 20.97로 증가폭이 2.75배·1.91배로 완만해졌다. 챕터 1 수치는 Task 14-1 기준값(능동 482.8초, 아이들 983.6초)에서 변하지 않았다. 각 챕터에서 보스 실패 1회 관문이 유지된다. 4시간 오프라인 보상은 챕터 3 기준 633,190 Credits로 같은 시점 드릴 강화 비용 48,001 대비 복귀 세션의 의미를 유지해 상한 4시간은 그대로 두었다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 196/196 통과. 신규 `ChapterProgressionBalanceTests` 7종과 전면 광고 유예 회귀 1종을 추가했고, 실제 카탈로그 수치를 고정하던 `MiningPowerServiceTests`의 보스 게이트 기대값을 새 값으로 갱신했다. Android 검증은 기본 검증 정책에 따라 미실행.
- 남은 작업: 챕터 2~3 전용 광석 아트를 도입하려면 박물관 그리드를 3행 이상으로 확장해야 한다. 장비·연구·도감 기여도는 현재 곡선에서 목표 범위를 만족해 이번에 변경하지 않았다. 다음은 최종 최적화와 QA다.

## 수정사항 73

- 기록 시각: 2026-08-06 20:40:00
- 작업 요청 요약: Task 16 최적화와 QA — 용량, 메모리, 프레임, 저장·업데이트 호환을 점검하고 개선한다.
- 측정 우선: HUD 전체 `Render()`를 300회 반복 측정해 0.099ms/호출임을 확인했다. 매 프레임 미션 보드를 LINQ로 재계산해 프레임을 갉아먹을 것이라는 최초 가정은 근거가 없었고(2회 호출 0.007ms), 렌더 경로는 최적화 대상이 아니라고 판단했다. 대신 `Resources` 폴더가 참조 여부와 무관하게 전량 빌드에 포함된다는 점에 착안해 UI 텍스처 25.10MB를 감사했다.
- 발견: 모든 스크립트의 문자열 리터럴을 수집해 대조한 결과 어디서도 이름으로 부르지 않는 텍스처가 27개(3.88MB) 있었다. 대부분 Task 13 UI 정리에서 `UiMuseumExhibitCardClean` 등 `...Clean` 자산으로 교체된 이전 원본이고, `29_SelectedNavigationTab`은 사용자가 제거한 선택 탭의 잔여 자산이다. 또한 Task13 텍스처 75개 전부에 Android 임포트 오버라이드가 없어 압축 포맷이 지정되지 않은 상태였다(Final 52개·V5 38개는 이미 ASTC_6x6).
- 수정한 내용: 미참조 텍스처 27개를 삭제했다(모두 git 추적 상태임을 먼저 확인했고 원본은 `Art/`에 남아 있다). 남은 Task13 텍스처 55개에 `ASTC_6x6`·CompressedHQ Android 오버라이드를 적용하되 최대 크기를 원본 이상의 2의 거듭제곱으로 지정해 화질 저하 없이 포맷만 바꿨다. Task 15 P2에서 상점·이벤트만 처리했던 하드코딩 영문 `CORE`가 피드백 토스트 3곳에 남아 있던 것을 `blueprint_core_short` 키로 마저 현지화했다.
- 수정 후 상태: Resources 소스는 25.10MB → 21.22MB다. Task13 텍스처 7.5MPixel 기준 압축 용량은 8bpp 환산 7.15MB에서 ASTC 6x6 3.18MB로 약 3.97MB 줄어드는 것으로 계산된다. 전체 Resources 텍스처의 ASTC 6x6 환산은 약 7.02MB다.
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 213/213 통과. 신규 `ResourceBudgetTests` 3종으로 미참조 텍스처 금지·명시적 ASTC 포맷·9MB 압축 예산을 고정했고, 신규 `SaveCompatibilityTests` 14종으로 저장 버전 1~12가 각각 `{"version":N}`만 남은 최악의 구버전 상태에서도 플레이 가능하게 로드되는지와 손상값 정규화·`Normalize` 멱등성을 검증했다. 자산 삭제 후 장비·박물관 1440×3088 캡처로 스프라이트 누락 0건을 확인했다. 자산 개수를 고정하던 기존 테스트 2종은 실제로 사용하는 자산 기준으로 갱신했다.
- 미검증: 실제 AAB 용량 절감은 계산에 따른 추정치다. 기본 검증 정책상 Android 빌드를 실행하지 않았으므로 직전 57.82MB 대비 실제 감소폭은 서명 AAB 재빌드로 확인해야 한다. 실기기 프레임·발열·메모리도 미측정이다.
- 남은 작업: 서명 Release AAB 재빌드로 용량 확인, 실기기 성능 측정, 이후 Jenkins 자동화.

## 수정사항 74

- 기록 시각: 2026-08-06 21:20:00
- 작업 요청 요약: Jenkins 기반 빌드 파이프라인을 구성한다. 채용 시장이 요구하는 "빌드 파이프라인 운영·개선 및 CI/CD 관리 경험"에 대응할 수 있는 형태로 만든다.
- 도입 근거: 이번 개발 중 실제로 두 번, 테스트 코드의 컴파일 오류로 `Assembly-CSharp-Editor` 빌드가 실패했는데 Unity 에디터가 직전에 성공한 DLL을 유지해 Test Runner가 옛 테스트를 실행하고 `184/184 통과`를 초록불로 보고했다. 새로 작성한 테스트는 실행조차 되지 않았고 Console에서 `error CS7036`을 직접 확인하고서야 발견했다. 로컬 초록불이 검증을 보장하지 못한다는 실제 사례이므로 이를 막는 것을 파이프라인의 1차 목표로 삼았다.
- 수정 전 상태: `PocketForgeAndroidBuild`가 환경변수 기반 서명 빌드를 제공했지만 `versionCode`가 1로 하드코딩돼 있었고, 파이프라인 정의·CI 진입점·문서는 없었다. Jenkins도 설치돼 있지 않다.
- 수정한 내용: 저장소에 `Jenkinsfile`을 추가해 Prepare → Compile → EditMode tests → Signed AAB → Report size 5단계를 선언형으로 정의했다. 컴파일을 테스트와 별도 스테이지로 분리해 실패 원인이 스테이지 이름만으로 드러나게 했다. `PocketForgeCi.AssertScriptsCompiled` 진입점을 추가해 `scriptCompilationFailed`, `Assembly-CSharp-Editor` 로드 여부, 카나리 테스트 타입 해석 여부 세 가지를 검사하고 하나라도 어긋나면 종료 코드 1을 반환하게 했다. 카나리 검사가 stale 어셈블리 탐지의 핵심이다. `PocketForgeAndroidBuild`가 `POCKETFORGE_VERSION_CODE`·`POCKETFORGE_VERSION_NAME`을 읽도록 바꿔 파이프라인이 `BUILD_NUMBER`를 주입하게 했고, 환경변수가 없으면 체크인된 기본값을 써 로컬 빌드 동작은 유지했다. 키스토어와 비밀번호는 `withCredentials`로만 주입하며 저장소에 두지 않는다. AAB 용량을 측정해 50MiB 초과 시 빌드를 `UNSTABLE`로 표시한다. 배경·스테이지 구성·비밀정보·노드 요구사항·개선 기록을 `CI.md`에 정리했다.
- 수정 후 상태: 파이프라인 정의와 진입점이 저장소에서 버전 관리된다. 진입점 판정 로직은 에디터에서 검증했다(카나리 타입 해석 성공, 테스트 타입 45개 확인, 두 진입점 메서드 존재 확인).
- 테스트 결과: Unity 컴파일 오류 0건, 전체 EditMode 213/213 통과.
- 미검증: Jenkins 설치와 실제 파이프라인 실행은 수행하지 않았다. 개발 PC의 Unity 에디터가 프로젝트를 점유하고 있어 배치모드 실행이 `another Unity instance is running`으로 실패함을 확인했으므로, 빌드 에이전트는 프로젝트를 단독 점유하는 별도 워크스페이스가 필요하다. Jenkins 설치는 시스템 변경이므로 승인 후 진행한다.
- 남은 작업: Jenkins 설치·자격증명 등록·파이프라인 첫 실행, 이후 Library 캐시로 빌드 시간 단축과 용량 추이 기록을 개선 이력으로 누적한다.

## 수정사항 75

- 기록 시각: 2026-08-07 21:40:00
- 작업 요청 요약: Jenkins 빌드 파이프라인을 실제로 구축·기동하고 서명 AAB까지 완주시킨다.
- 도입 근거: 개발 중 두 번, 테스트 코드 컴파일 실패 시 Unity가 직전 DLL로 테스트를 돌려 위양성 초록불을 보고한 사고가 있었다. 이번 세션에서 이를 의도적으로 재현해 검증했다. 컴파일 오류가 있는 파일과 `1 == 2`를 단언하는 테스트를 넣었는데도 Test Runner는 `213/213 통과`를 보고했고, CI 검사(`scriptCompilationFailed` + 카나리 타입 해석)는 종료 코드 1로 잡았다.
- 환경 구성: Jenkins LTS 2.568.2를 WAR로 8081 포트에 기동했다. 기본 8080은 Unity MCP 브리지가 점유 중이라 그대로 쓰면 에디터 제어 연결이 끊긴다. Jenkins LTS가 Java 17 지원을 중단해 Temurin JDK 21을 설치했다(Unity Android 빌드는 자체 JDK를 쓰므로 무관). NUnit 플러그인은 업데이트 센터 다운로드가 TLS 리셋으로 반복 실패해, curl로 받아 의존성과 Jenkins 버전 요구사항을 확인한 뒤 수동 배치했다.
- 파이프라인이 검출한 결함: 빌드 #2에서 로컬 213/213 초록불 상태인데도 XP 라벨의 BestFit 하한이 12로 기준 18을 위반함을 검출했다. 빌드 #3에서 같은 종류의 라벨이 68개임을 전수 조사로 확인했고, 67개는 18로 올려도 여백이 남고 실제 문제는 스타터 패키지 값 하나뿐임을 측정했다. 리터럴 10곳을 상향하고 공통 헬퍼 3곳에서 `Mathf.Clamp`로 하한을 강제해 호출부가 무엇을 넘기든 규칙이 지켜지도록 구조를 바꿨다. 덤으로 영어 문자열 잘림 2건도 발견해 수정했다.
- 최종 결과: 빌드 #5가 Prepare → Compile → EditMode tests → Signed AAB → Report size 5단계를 완주했다. 전체 5분 3초, 테스트 213/213 통과, `jarsigner -verify` = `jar verified`. AAB 용량은 54,550,979 bytes(52.02 MiB)로, 이전 수동 빌드 57,822,260 bytes(55.14 MiB) 대비 3.12 MiB(−5.7%) 줄었다. Task 16에서 계산했던 추정 절감 약 4MB를 실측으로 대체했다. 50MiB 예산을 2.02MiB 초과해 의도대로 `UNSTABLE`로 표시됐다.
- 측정된 개선: Unity 임포트가 첫 실행 150초에서 Library 캐시 재사용 시 22초로 85% 단축됐다.
- 테스트 결과: 로컬 전체 EditMode 213/213 통과, CI 빌드 #5에서도 213/213 통과.
- 남은 작업: SCM을 GitHub 원격으로 전환해 `ALLOW_LOCAL_CHECKOUT` 우회를 제거하고, 용량·빌드 시간 추이를 시각화하며, `main` 푸시 자동 트리거와 실패 알림을 붙인다. 용량은 50MiB 목표까지 2.02MiB 남았다.

## 수정사항 76

- 기록 시각: 2026-08-08 22:10:00
- 작업 요청 요약: Task 17 운영 개선. 로컬 디렉터리 SCM 우회를 제거하고, 용량 추이 시각화와 `main` 푸시 자동 트리거를 붙인다.
- 수정 전 상태: 파이프라인이 개발 PC의 프로젝트 디렉터리를 직접 체크아웃했다. Jenkins가 이를 막기 때문에 `ALLOW_LOCAL_CHECKOUT` 시스템 프로퍼티로 보안 검사를 꺼 두어야 기동됐고, 빌드는 수동 실행뿐이었으며 용량은 로그의 한 줄로만 남았다.
- 수정한 내용: SCM을 GitHub 원격으로 전환해 Jenkins가 자체 워크스페이스에 클론하도록 바꾸고 `ALLOW_LOCAL_CHECKOUT`을 제거했다. 이 전환에는 부수 효과가 하나 더 있다. 개발 PC의 Unity 에디터가 프로젝트를 점유하면 배치모드가 `another Unity instance is running`으로 실패하는데, 별도 워크스페이스는 이 잠금과 무관하므로 에디터를 켠 채로도 CI가 돈다. `pollSCM('H/10 * * * *')`으로 `main`이 실제로 움직였을 때만 빌드가 시작되게 했다. 컨트롤러가 GitHub에서 도달 불가라 웹훅 대신 폴링을 쓴다. Plot 플러그인으로 AAB 용량을 빌드별 선형 추이로 기록하고, Prepare 단계에서 재사용 워크스페이스에 남은 이전 `*.aab`·`size.csv`를 먼저 지우도록 했다. 빌드 #6의 아카이브에 5번과 6번 AAB가 함께 담긴 것을 보고 잡은 결함이다. 용량 읽기는 `new File()`이 Groovy 샌드박스에서 거부되므로 `fileExists` + `bat` + `readFile` 조합으로 바꿨다.
- 수정 후 상태: 푸시 후 30초 이내에 폴링이 변경을 감지해 빌드가 자동 시작되는 것을 확인했다. 빌드 #7(42초)·#8(73초) 모두 SUCCESS.
- 테스트 결과: CI 빌드 #7·#8에서 EditMode 214/214 통과.
- 미검증: 실패 알림(email/webhook)은 아직 붙이지 않았다.
- 남은 작업: 실패 알림 연결.

## 수정사항 77

- 기록 시각: 2026-08-08 22:40:00
- 작업 요청 요약: 스토어 업로드 준비의 첫 단계로 실기기 스모크 테스트를 수행한다.
- 도입 근거: Task 16에서 55개 텍스처를 ASTC_6x6으로 재임포트했다. 압축 포맷 변경은 에디터에서 보이지 않다가 기기에서만 드러나는 종류의 위험이라 실기기 확인이 필요했다.
- 수행 내용: CI 빌드 #6이 만든 AAB를 bundletool로 디버그 서명 유니버설 APK로 변환해 SM-S938N(`R3CY3094K9Y`)에 설치했다. 기기에서 `versionCode=6`을 확인해 파이프라인의 `BUILD_NUMBER` 주입이 실제 산출물까지 도달함을 검증했다. `adb shell input tap` 루프로 챕터 1 보스까지 진행해 메인 HUD, 설정, 장비, 챕터 클리어, 박물관을 캡처했다(`Review/device/*.png`).
...
- 기록 시각: 2026-08-08 23:55:00
- 작업 요청 요약: 스토어 업로드 준비 3단계. Play Console 등록정보를 작성한다.
- 사용자 결정: 개인정보처리방침은 GitHub Pages에 게시, 문의 이메일은 개인 Gmail 대신 별도 주소로 교체, 배포 국가는 대한민국으로 제한.
- 작성한 내용: `docs/store-listing.md`에 앱 이름·간단한 설명·자세한 설명을 한국어·영어로 작성하고 각 항목의 글자 수를 Play 상한과 함께 적었다. 데이터 보안 양식 답변은 새로 판단하지 않고 수정사항 78의 실측 근거를 그대로 사용했다(수집·공유는 광고 SDK의 기기·광고 ID와 앱 활동뿐, 개발자 수집 0건, 충돌·성능 리포팅 비활성, 삭제 경로는 앱 삭제). 콘텐츠 등급 예상 답변과 필요한 그래픽 자산 목록도 정리했다.
- 배포 국가 제한의 근거: UMP 미구현 상태에서 EEA·영국에 광고를 게재하면 Google EU 사용자 동의 정책을 만족하지 못한다. 외부 SDK 추가는 승인 대상이라, 비공개 테스트 단계에서는 국가 제한으로 대응하기로 했다.
- 게시를 보류한 이유: 저장소가 PUBLIC이라 Pages를 켜면 방침 페이지가 색인된다. 문의 이메일이 확정되기 전에 켜면 개인 Gmail이 렌더링된 페이지로 노출되고 되돌리기 어려우므로, 주소 확정 전까지 활성화하지 않는다.
- 확인된 자산 공백: 앱 아이콘 512×512와 피처 그래픽 1024×500이 없다. 기기 캡처 5장은 1440×3120으로 세로비가 2.167:1인데 Play의 휴대전화 스크린샷 종횡비 상한(9:16 = 1.78:1)을 넘을 가능성이 높아 재구성이 필요하다. 실제 허용 여부는 등록 시 확인한다.
- 미검증: Play Console에 실제로 입력하지 않았다. 문구 길이는 계산했으나 Play 편집기의 카운트 방식과 일치하는지는 확인하지 않았다.
- 남은 작업: 문의 이메일 확정 → 방침 게시 → 아이콘·피처 그래픽·스크린샷 준비 → 내부 테스트 업로드.

## 수정사항 80

- 기록 시각: 2026-08-09 00:30:00
- 작업 요청 요약: 스토어 그래픽 자산을 준비한다. 문의 이메일은 기존 Gmail을 그대로 사용하기로 확정됐다.
- 확인한 사실: `ProjectSettings.asset`의 `m_Icons`가 iPhone·Android·기타 플랫폼 모두 `m_Textures: []`였다. 즉 앱 아이콘이 한 번도 지정된 적이 없고, 지금까지의 모든 빌드는 Unity 기본 아이콘으로 기기에 설치됐다.
- 종횡비 요건 확인: 스크린샷 재구성 전에 Play 공식 문서를 조회했다. 처음에 가정했던 9:16은 추천작 선정 자격의 *권장* 값이고, 하드 제한은 "긴 변이 짧은 변의 2배를 넘을 수 없다"였다. 기기 캡처 1440×3120은 2.167:1로 이 하드 제한을 넘어 실제로 거부된다. 가정 위에 작업을 쌓기 전에 확인해서 잘못된 크기로 재작업하는 일을 피했다.
- 작성한 자산 (`docs/store-assets/`): 앱 아이콘 512×512(229KB, 32비트 PNG), 정사각 런처 아이콘 432×432, 피처 그래픽 1024×500(24비트, 알파 없음), 스크린샷 5장 1440×2880(정확히 2.0:1). 스크린샷은 아바타 위 배경만 있는 상단 240px을 잘라 UI 손실 0으로 맞췄고 결과를 육안 확인했다.
- 자산 제작 방식: 새 이미지를 생성하지 않고 기존 게임 아트(곡괭이 아이콘, 금 광석 아이콘)를 합성했다. 배경색은 임의로 고르지 않고 스크린샷에서 직접 추출했다(동굴 파랑 `#21357b`, 액션 금색 `#fdb916`). 피처 그래픽 1차안은 곡괭이가 오른쪽 끝에서 잘리고 왼쪽 60%가 비어 있어 중앙 정렬로 다시 만들었다. 텍스트는 넣지 않았다. Play는 등록정보 언어마다 피처 그래픽을 따로 받으므로 문구를 구우면 한국어·영어 두 벌을 유지해야 한다.
- 미해결: 인게임 런처 아이콘 설정은 Unity Player Settings 변경이라 에디터가 필요하다. Adaptive icon 전경(중앙 66% 안전 영역)·배경 분리 레이어도 미제작이다. 현재 자산은 배경이 포함된 정사각 이미지다.
- 남은 작업: Pages 활성화(사용자), 런처 아이콘 설정, 내부 테스트 업로드.

## 수정사항 81

- 기록 시각: 2026-08-09 02:00:00
- 작업 요청 요약: 남은 작업(런처 아이콘, 실기기 성능 측정, 빌드 실패 알림)을 마무리한다.
- **파이프라인 결함 발견**: `Signed AAB`와 `Report size` 단계가 빌드 #7부터 #15까지 9회 연속 건너뛰어졌는데 전부 SUCCESS로 보고됐다. 원인은 `when { branch 'main' }`이 멀티브랜치 파이프라인에서만 평가된다는 점이다. 이 잡은 단일 브랜치라 `BRANCH_NAME`이 비어 조건이 한 번도 참이 아니었다. 빌드 #5·#6은 `BUILD_ANDROID` 파라미터를 체크한 수동 실행이라 통과했고, 내가 추가한 폴링 트리거는 파라미터 기본값 `false`를 쓴다. 즉 자동 트리거 도입이 패키징을 멈춘 원인이었다.
- 발견 경위: 테스트 결과나 로그가 아니라 빌드 소요시간이었다. #5 302초·#6 182초에서 #7 이후 42~48초로 떨어진 것을 이상하게 여겨 역추적했다. 초록불만 확인했다면 계속 놓쳤을 결함이다.
- 수정: 브랜치를 `BRANCH_NAME` → `GIT_BRANCH` → 기본값 순으로 해석하도록 바꿨다. 이 잡의 모든 빌드가 main이므로 기본값은 패키징이다. 그리고 패키징했어야 하는 빌드가 AAB를 만들지 않으면 SUCCESS 대신 UNSTABLE로 표시하는 가드를 추가해 같은 종류의 조용한 누락이 반복되지 않게 했다. 빌드 #16에서 178초·`PocketForge-16.aab` 생성으로 복구를 확인했다.
- **런처 아이콘**: `ProjectSettings.asset`의 `m_Icons`가 전 플랫폼 비어 있어 지금까지 모든 빌드가 Unity 기본 아이콘으로 나갔다. 기존 게임 아트로 Adaptive 전경·배경, Round, Legacy 레이어를 만들고 `PlayerSettings` API로 지정했다. 릴리스 빌드가 매번 재적용해 에디터 UI를 연 적 없는 CI 워크스페이스에서도 실제 아이콘이 들어간다.
- **아이콘 1차 시도는 회귀였다**: 빌드 #16 AAB를 열어 대조하니 `app_icon.png`는 6개 밀도로 들어갔지만 `mipmap-anydpi-v26` XML과 foreground/background 레이어가 사라졌다. Unity 로그는 `icon kinds assigned: Legacy(6)`였다. `"Adaptive"`·`"Round"` 문자열 매칭이 둘 다 실패해, 기본값이 갖고 있던 adaptive 아이콘을 오히려 제거한 것이다. 실제 이름은 `"Adaptive (API 26)"`, `"Round (API 25)"`였다.
- 재수정: 이름 대신 `icon.maxLayerCount`로 판정한다. 레이어 2개를 요구하는 종류가 adaptive다. 로그도 지원 종류 수와 슬롯·레이어 수를 출력하게 해 AAB를 뜯지 않아도 확인되게 했다. 빌드 #17에서 `Adaptive (API 26)[6 slots, 2 layers] | Round (API 25)[6 slots, 1 layers] | Legacy[6 slots, 1 layers]` 전부 적용을 확인했다.
- 빌드 #17 AAB 실측 대조 (빌드 #6 기본값 → #17): `app_icon.png` 1종 926B → 6종 91,936B, `app_icon_round.png` 없음 → 6종 91,591B, `ic_launcher_foreground` 1종 1,612B → 6종 157,352B, `ic_launcher_background` 1종 101B → 6종 167,290B, anydpi-v26 XML 1개 → 2개.
- **빌드 실패 알림**: `failure`/`unstable`/`fixed` 상태 전이에서만 발신한다. 초록불이 이어지는 동안은 조용하고 첫 실패만 알린다. 웹훅·메일 모두 노드 환경변수가 있을 때만 동작해 설정 없는 머신에서도 파이프라인은 그대로 돈다. JSON은 Groovy 샌드박스가 `JsonOutput`을 막으므로 직접 조립했다.
- **실기기 성능 측정** (SM-S938N, 9분 연속 탭 부하): 6,435 프레임 중 놓친 프레임 2(0.03%), present-to-present 16ms 85.7%로 60fps 고정, 평균 프레임 지속 0.805ms, 배터리 온도 32.9C 변동 0, PSS 407.8MB → 398.7MB 안정. `dumpsys gfxinfo`가 `Total frames rendered: 0`을 낸 것은 HWUI만 집계하기 때문으로, Unity처럼 SurfaceView에 직접 그리는 앱은 `SurfaceFlinger --timestats`로 측정해야 한다. 발열은 USB 충전 중 측정이라 보수적인 값이다.
- 미검증: 빌드 #17 APK를 기기에 설치해 런처 아이콘을 눈으로 확인하려 했으나 기기 연결이 끊겨(`no devices/emulators found`) 수행하지 못했다. APK는 생성해 두었고 재연결 시 설치만 하면 된다. AAB 리소스 수준 검증은 완료했다.
- 남은 작업: 기기 재연결 후 런처 아이콘 육안 확인, Pages 활성화·알림 채널 등록·실 광고 ID·내부 테스트 업로드(모두 사용자 조치).

## 수정사항 82

- 기록 시각: 2026-08-09 07:10:00
- 작업 요청 요약: 실 광고 ID 적용, 스토어 자산 규격 재확정, 기기 재연결 후 미검증 항목 해소.
- 실 광고 ID: 앱 ID `ca-app-pub-9102736521930389~3693750260`, 보상형 `/6758897305`, 전면 `/3318345744`. 두 단위 중 어느 쪽이 보상형인지는 ID로 판별할 수 없어 사용자에게 확인한 뒤 적용했다. 바꿔 넣으면 보상 지급 위치에서 전면 광고가 뜨는데 광고 자체는 재생되므로 스모크 테스트로 걸러지지 않는다.
- 신규 `ReleaseAdIdentifierTests` 3종: 앱 ID·광고 단위가 Google 테스트 퍼블리셔가 아닌지, 두 placement가 서로 다른 단위인지 검사. 상수가 `UNITY_ANDROID` 뒤에 있어 테스트 실행 시 정의되지 않을 수 있으므로 소스 텍스트로 검사한다.
- 스크린샷 규격 재확정: 처음 Play 도움말의 하드 제한("긴 변이 짧은 변의 2배 이하")만 보고 1440×2880(2.0:1)으로 만들었으나, Play Console 등록정보 폼은 비율을 16:9 또는 9:16으로 명시한다. 정확히 9:16(1440×2560)으로 다시 만들어 논쟁을 없애고 프로모션 자격 조건("4개 이상 중 3개는 9:16에 1,080px 이상")도 함께 충족시켰다. 하단 내비게이션 바를 잃지 않도록 크롭 대신 전체 화면을 높이에 맞춰 축소하고 좌우 129px 여백은 같은 화면의 cover·블러·감광 배경으로 채웠다. 첫 시도인 가장자리 컬럼 늘리기는 가로 줄무늬가 보여 기각했다.
- 데이터 보안 양식: 목적을 "광고 또는 마케팅" 하나로 안내했던 것을 Google 공식 안내 확인 후 3개(광고 또는 마케팅·분석·부정 행위 방지·보안 및 규정 준수)로 정정했다. 임시 처리는 "아니요"다. 임시 처리로 신고하면 등록정보에 표시되지 않으므로 보관되는 데이터를 감추는 잘못된 신고가 된다.
- 기기 검증 (빌드 #22, versionCode 22, 저장 데이터 유지 업그레이드 설치):
  - **런처 아이콘 확인 완료**. Squircle 마스킹된 adaptive icon으로 곡괭이·금 광석이 정상 표시된다. Unity 기본 아이콘이 아니다. 이전 세션에서 리소스 수준까지만 검증했던 항목을 해소했다.
  - **박물관 진행 아이콘 수정 확인 완료**. `4 / 4` 옆이 미발견 `?`가 아니라 초록 완료 배지다. 스토어 스크린샷도 이 빌드로 재캡처했다.
  - **광고 통합 경로 확인**. 실 광고 단위로 요청이 나가고 AdMob이 구조화된 응답을 반환했다. 결과는 `Code 3 / "Account not approved yet."`으로, 통합 결함이 아니라 AdMob 계정 승인 미완이다. 계정 승인 후 서빙이 시작된다.
  - AdMob 테스트 기기 ID `2C1684FC19DC2843EF4E829771E072F6`을 logcat에서 추출했다. 실 단위로 기기 테스트 전 AdMob에 등록해야 무효 트래픽 정지를 피한다.
  - 크래시 0건.
- 발견한 미해결 항목: 런처에 표시되는 앱 이름이 `productName`을 그대로 써서 `PocketForge`(공백 없음)로 나온다. Play 스토어 이름과 별개로 기기에 보이는 이름이며, 최소한 공백을 넣거나 언어별 런처 이름을 Android 리소스로 현지화할지 결정이 필요하다.
- 프로덕션 요건 확인: Play Console이 프로덕션 액세스에 비공개 테스트 테스터 12명 이상·14일 이상을 요구한다. 내부 테스트는 이 요건에 카운트되지 않는다.

## 수정사항 83

- 기록 시각: 2026-08-09 14:30:00
- 작업 요청 요약: 런처에 표시되는 앱 이름을 언어별로 현지화한다.
- 수정 전 상태: Unity가 `productName`을 런처 모듈의 기본 `strings.xml`에 `app_name`으로 써서, 4개 언어를 지원하는 게임인데 홈 화면 이름만 모든 언어에서 `PocketForge`(공백 없음)로 고정돼 있었다.
