...
| 2026-08-10 | 업로드 실패한 버전 코드는 재사용 불가임을 기록 | Play Console은 한 번 업로드된 `versionCode`를 영구히 소모한다. 릴리스 초안을 삭제해도 마찬가지다. 업로드가 중간에 실패해 번들이 붙지 않은 초안이라도 번호는 이미 쓰인 것으로 잡히므로, 재시도할 때는 반드시 새 빌드를 돌려 번호를 올려야 한다 |
