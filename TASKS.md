...
현재 작업: Task 12 HUD 실기기 비율 보정 적용 — 사용자 기기 화면 재확인 대기

- 완료: 설정창은 승인 상태로 동결하고 HUD 코드만 수정
- 완료: 곡괭이·드릴·로봇 `RawImage`를 원본 텍스처 종횡비 안에서 맞춰 강제 늘어남 제거
- 완료: 광석 이름 배지를 진행 바보다 앞에 렌더링해 겹침으로 생기던 눌림·돌출 제거
- 완료: 강화 카드의 레벨 핍·레벨·비용·액션 버튼을 독립 행으로 분리하고 비용 아이콘·숫자 겹침 제거
- 완료: 채굴 보상·상태 피드백 패널을 제거하고 캐주얼 굵은 글꼴 우선순위, 두꺼운 외곽선·그림자, 짧은 팝·상승 페이드 텍스트 적용
- 검증: Unity 컴파일 오류 0건, EditMode 48/48 통과
- 제한: 사용자 요청에 따라 이번 패스의 Android/APK 빌드와 실기기 검증은 생략
- 다음: 사용자가 돌아오면 최신 변경분을 실기기에서 캡처해 HUD 비율과 보상 텍스트를 최종 확인

- 완료: 853×1844 승인 스냅샷을 1080 기준 캔버스로 환산해 HUD 상단 바·설정·광고·광석 게이지·채굴·강화 카드와 설정 모달 외곽·행·슬라이더·닫기 버튼의 좌표·크기 재보정
- 완료: 승인 좌표의 회귀를 막는 `ApprovedSnapshotChrome_UsesMeasured1080WideGeometry` EditMode 테스트 추가
- 검증: 원본과 1440×3120 SM-S938N 캡처를 동일 해상도로 축소·반투명 오버레이해 주요 프레임 경계가 포개지는지 확인
- 검증: Unity 컴파일 오류 0건, EditMode 45/45 통과, `.dev` 비개발 APK 50,050,511바이트(47.73MiB) 빌드·설치·기동, 채굴 `10/10 → 09/10`, 설정창 표시 확인
- 참고: 원본 설정의 지구본 중심 언어 행은 실제 요구사항인 한국어·영어·일본어·중국어 4개 버튼으로 유지해 의도적으로 동일하지 않음
