...
현재 작업: 광고 실기기 완료 검증 — 보상형 광고 재생·보상 지급 및 전면 광고 정책 확인
...
- 보류: 클릭 애니메이션, 업그레이드 문구 대체 이펙트, 전용 폰트는 나머지 기능 구현 후 최종 폴리싱 단계에서 진행
...
- 다음: 광고 실기기 검증을 닫은 뒤 Play Console의 `remove_ads` 상품·라이선스 테스터·내부 테스트 트랙에서 실제 구매·복원을 검증
...
- UI 폴리싱은 광고와 인앱 결제가 모두 완료되기 전에는 시작하지 않는다.
